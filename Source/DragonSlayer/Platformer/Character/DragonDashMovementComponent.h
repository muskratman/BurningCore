...
private:
	...
	void RestoreAerialDecisionTimeDilation();
	...
	bool HasHitEnemyTargetInCurrentDash(AActor* TargetActor) const;
	void RememberEnemyTargetHit(AActor* TargetActor);
	void EnterEnemyBounce(AActor* TargetActor, const FHitResult& Hit, UDragonDashEnemyTargetComponent* TargetComponent);
	FVector ResolveEnemyBounceIncomingDirection(AActor* TargetActor) const;
	FVector BuildEnemyBounceDirectionFromInput() const;
	void RefreshEnemyBounceDirectionFromInput();
	...
	float ActiveAerialDecisionTimeDilation = 1.0f;
	float ActiveEnemyHitStopTimeDilation = 1.0f;
	bool bActiveDashStartedInAir = false;
	bool bActiveDashFromEnemyBounce = false;
	bool bPendingAerialDecisionAfterEnemyBounce = false;
	bool bAerialDecisionTimeDilationActive = false;
	...
	TArray<TWeakObjectPtr<AActor>> EnemyTargetsHitThisDash;
	FVector PendingEnemyBounceIncomingDirection = FVector::ForwardVector;
	FTimerHandle AerialDecisionTimerHandle;
