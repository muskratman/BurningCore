...
This project is a **2.5D side-scrolling platformer** set in a **magical / fairytale fantasy world**. The world is built in **3D**, but the camera shows it from a **side-view perspective**. The main visual reference is **Heroes of Might and Magic II: The Succession Wars**, but **without pixel art**.

The visual direction should feel like **bright fairytale fantasy**, with a strong **storybook illustration** quality, rich color harmony, and a slightly theatrical, charming, handcrafted feel.

The assets and environments should be rendered as **hand-painted 3D models**, so they feel more **flat, illustrated, and painterly** rather than highly realistic.

For level construction and camera depth, use:

- **Ori and the Blind Forest** as a reference for layered composition, camera staging, and scenic depth
- **Inside** as a reference for level depth, background functionality, and readable environmental structure

---

## Core Style Rules

These rules should remain consistent across all prompts.

- Bright fairytale fantasy
- Storybook illustration feel
- Heroes of Might and Magic II inspired
- Hand-painted 3D surfaces
- Not pixel art
- Slightly flattened painterly look
- Rich but clean colors
- Clear silhouettes
- Layered side-view composition
- Atmospheric depth
...
Use these variables in prompts to keep the system modular.
...
```text
Design the far background layer for a 2.5D side-scrolling fantasy scene set in {BIOME}. The background is the distant backdrop of the level and should provide scale, atmosphere, and horizon depth without competing with the playable space.

The background should be composed as a very wide plan with large distant forms only: mountain ranges, far cliffs, massive silhouettes, distant treelines, broad sky gradients, cloud masses, mist layers, smoke, and other far-off horizon elements appropriate to the biome. Keep the background simplified, atmospheric, and painterly, with soft depth separation and subtle detail.

The background must support the fairytale fantasy direction of the project and reinforce the classic painted fantasy game-art feel. It should read as a living distant backdrop with strong world scale, but remain clearly non-interactive and visually behind the midground.

Avoid busy detail, harsh contrast, or anything that pulls attention away from the playable space.
```
...
Use this order when assembling a final prompt:
